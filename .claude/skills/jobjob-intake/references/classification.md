...
폴더명은 `<YYYY-MM>-<회사>-<직무>`.
- `<YYYY-MM>`은 정리일 기준이다. 접수 기간이 아니다.
- `<직무>`는 공고의 직무명에서 공백을 빼고, "신입"·"경력"·"채용" 같은 구분어는 뺀다. 예: "백엔드 개발 신입" → `백엔드개발`.
- 같은 회사·직무 폴더가 이미 있으면 posting.md를 덮어쓰지 말고 `posting-<날짜>.md`로 저장하고 보고한다.

문항 유형 열에 한 문항이 두 유형에 걸치면 `지원동기+포부`처럼 `+`로 잇는다. 앞이 주 유형이다.
출처가 URL이 아니면 파일명이나 "채팅 붙여넣기 <날짜>"로 적어 추적할 수 있게 한다.
